...
class UserAdmin(BaseUserAdmin):
    ...
    list_display = ['handle', 'avatar_color', 'is_shadowbanned', 'report_count', 
                   'date_joined', 'last_post_time', 'is_anonymous_mode']
    list_filter = ['is_shadowbanned', 'is_anonymous_mode', 'date_joined']
    ...
    fieldsets = (
        ...
        ('Moderation', {'fields': ('is_shadowbanned', 'shadowban_reason', 'report_count')}),
        ('Rate Limiting', {'fields': ('last_post_time', 'posts_count_today')}),
        ('Permissions', {'fields': ('is_active', 'is_staff', 'is_superuser')}),
        ('Important dates', {'fields': ('last_login', 'date_joined')}),
    )
    
    actions = ['shadowban_users', 'remove_shadowban', 'reset_report_count']
    
    def shadowban_users(self, request, queryset):
        """Shadowban selected users"""
        count = queryset.update(is_shadowbanned=True, shadowban_reason="Admin action")
